...
const performWebSearch = (query, filters) => {
 ...
 */
const performWebSearchWithCache = async (userId, query, options = {}) => {
 ...
 */
const createSearchHistory = async (userId, historyData) => {
 ...
 * @returns {Promise<SearchHistory>} Created search history
 */
const saveSearchToHistory = async (userId, searchData) => {
    return await prisma.searchHistory.create({
        data: {
            userId,
 ...
 */
const getUserSearchStats = async (userId) => {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const thisWeek = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    const thisMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const [totalSearches, uniqueQueries, avgResult, mostSearched, todayCount, weekCount, monthCount] = await Promise.all([
 ...
 */
const getSystemSearchStats = async () => {
